...
@Controller  // it handles https verbs
public class SignUpController {

    @Autowired // to acheive dependency injection
    private UserRepository userRepository;

    @GetMapping("/signup") // to show the signup form
    public String showSignupForm(Model model) {
        model.addAttribute("user", new User());
        return "signup";
    }

    @PostMapping("/signup") // to handle form submission
    public String submitSignupForm(@Valid User user, BindingResult result, Model model) {
        ...
        model.addAttribute("success", true); // to display success message
        model.addAttribute("name", user.getName()); // to display name
        return "signup";
    }
}
